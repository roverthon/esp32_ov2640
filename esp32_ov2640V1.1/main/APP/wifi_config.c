...
/* 链接wifi名称 */
#define DEFAULT_SSID "1234561"
/* wifi密码 */
#define DEFAULT_PWD "one23456one"
/* 事件标志 */
...
void connet_display(uint8_t flag)
{
    if ((flag & 0x80) == 0x80)
    {
        lcd_fill(0, 90, 320, 240, BLACK);
        sprintf(lcd_buff, "热点名字:%s", DEFAULT_SSID);
        lcd_display_string(0, 90, WHITE, BLACK, 16, lcd_buff);

        sprintf(lcd_buff, "连接密码:%s", DEFAULT_PWD);
        lcd_display_string(0, 110, WHITE, BLACK, 16, lcd_buff);

        lcd_display_string(0, 130, WHITE, BLACK, 16, "按键0:发送数据");
    }
    else if ((flag & 0x04) == 0x04)
    {
        lcd_display_string(0, 90, WHITE, BLACK, 16, "wifi 连接中......");
    }
    else if ((flag & 0x02) == 0x02)
    {
        lcd_display_string(0, 90, WHITE, BLACK, 16, "wifi 连接失败");
    }
    else if ((flag & 0x01) == 0x01)
    {
        printf("%s\r\n", network_connet.ip_buf);
        lcd_display_string(0, 150, WHITE, BLACK, 16, (char *)network_connet.ip_buf);
    }
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        s_retry_num = 0;
        sprintf(network_connet.ip_buf, "分配ip:" IPSTR, IP2STR(&event->ip_info.ip));
        network_connet.fun(network_connet.connet_state);
