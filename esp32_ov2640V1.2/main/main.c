...
/*ov2640任务配置（独立，低优先级，120秒更新）*/
#define OV2640_TASK_PRIO       5
#define OV2640_TASK_STK_SIZE   8192        /* 天气任务需要更大栈空间 */
...
void ov2640_task(void *pvParameters)
{
    ...
    // if (ret1 == ESP_ERR_NVS_NO_FREE_PAGES ||  
    //      ret1 == ESP_ERR_NVS_NEW_VERSION_FOUND) 
    // { 
    //     ESP_ERROR_CHECK(nvs_flash_erase()); 
    //     ret1 = nvs_flash_init();    
    // } 
 ...
 lcd_clear(BLACK); 
  ...
  //  sensor_t *s = esp_camera_sensor_get();
    int cam_width = 80;   // 例如 320 (QVGA)
    int cam_height = 60; // 240
    while (1)
    {
        ...
        taskENTER_CRITICAL(&my_spinlock);
        lcd_display_picture(130, 130, cam_width, cam_height, fb->buf);
        taskEXIT_CRITICAL(&my_spinlock);
