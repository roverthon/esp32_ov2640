    ...
    vTaskDelay(pdMS_TO_TICKS(8000));
     ...
     while (1) {
        if (esp32_get_network_time(&time_info) == ESP_OK) {
            ...
            // 显示在左上角，y=10（不会覆盖右下角）
            lcd_display_string(0, 10, CYAN, BLACK, 16, date_buffer);
            xSemaphoreGive(display_1);
        } else {
            xSemaphoreTake(display_1, portMAX_DELAY);
            lcd_display_string(0, 10, YELLOW, BLACK, 16, "等待时间同步...");
            xSemaphoreGive(display_1);
        }
...
void display_weather_task(void *pvParameters)
{
    ...
    while (1) {
        if (esp32_get_weather(&weather_info) == ESP_OK) {
            ...
            xSemaphoreTake(display_1, portMAX_DELAY);
            lcd_display_string(0, 30, GREEN, BLACK, 16, weather_buffer);
            xSemaphoreGive(display_1);
            ESP_LOGI(TAG, "天气显示: %s", weather_buffer);
        } else {
            xSemaphoreTake(display_1, portMAX_DELAY);
            lcd_display_string(0, 30, RED, BLACK, 16, "天气获取失败");
            xSemaphoreGive(display_1);
            ESP_LOGW(TAG, "天气获取失败");
        }
