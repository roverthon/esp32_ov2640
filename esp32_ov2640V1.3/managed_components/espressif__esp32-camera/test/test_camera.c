...
static esp_err_t init_camera(uint32_t xclk_freq_hz, pixformat_t pixel_format, framesize_t frame_size, uint8_t fb_count, int sccb_sda_gpio_num, int sccb_port)
{
    ...
    .pin_sccb_sda = SIOD_GPIO_NUM,      // 直接用具体引脚
    .pin_sccb_scl = SIOC_GPIO_NUM,
    // .sccb_i2c_port 不再需要，因为上面不是 -1
    ...
    .xclk_freq_hz = 12000000,           // 固定 20MHz
    .ledc_timer = LEDC_TIMER_0,
    .ledc_channel = LEDC_CHANNEL_0,

    .pixel_format = PIXFORMAT_RGB565,     
    .frame_size = FRAMESIZE_QVGA,       // 先稳再高
    .jpeg_quality = 10,                 // 略微提高画质
    .fb_count = 2,                      // 有 PSRAM 就开 2
